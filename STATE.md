...
| Progress-tracking app v2 (user request 2026-09-04) | ✅ BUILT & PUSHED — tabs (Overview / Career paths), per-person green/yellow/red marking, sticky career trace (bug fixed), edges redraw on font-load/resize (bug fixed), career tab shows only path+enrichment nodes. Storage: localStorage fallback + Vercel/Neon API (api/ folder). GitHub: https://github.com/kaspar-i/kids-learning (public). USER ACTION NEEDED: Vercel import + Neon integration per DEPLOY.md, then /api/setup once |
