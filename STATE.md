...
| Progress-tracking app v2 (user request 2026-09-04) | ✅ LIVE — https://kids-learning-phi.vercel.app (Vercel project in Insly team + Neon Frankfurt, schema created via /api/setup). Tabs (Overview / Career paths), per-person ADDITIVE marking (mastered/learning; red state removed by owner decision after 3 international critics), sticky career trace, edges redraw on font-load/resize, career tab shows only path+enrichment nodes. API end-to-end tested 2026-09-04 (board/person/progress round-trip + bad-state rejection + cascade delete). GitHub: https://github.com/kaspar-i/kids-learning — every push auto-redeploys |
| v1.8 bundle rungs (user request 2026-09-05) | ✅ DONE — all 21 tier-4/5 bundles carry authored 3–7-step ladders (100 rungs) in the canonical JSON; app: steps badge on bundle cards, panel = bundle page with markable ladder (progress key nodeId/rungId), "Expand bundles" toggle unfolds ladders on the map in both tabs. Live + pushed |
| v1.7 + verification + charter alignment | ✅ DONE — v1.7 applied (42/42 findings, verified in docs/reviews/int-fix-verification.md); app aligned with CR19 (additive career counts, no bars/percents; correct Estonian tier labels; CR19 clause 8 states the parent-operated full-view deviation). Product strategy in docs/business/product-strategy.md (sell booklets first; Stage B/C signal-gated). All live + pushed |
| International critics (user request 2026-09-04) | ✅ ALL THREE DONE — docs/reviews/int-review-finland.md (1 blocking/9 important), int-review-singapore.md (1 blocking/8 important), int-review-japan.md (1 blocking/10 important). ALL THREE converge on the same blocking finding: child-facing red marks + adult-set states contradict the framework's own rulings (GP9, CR11/CR13, §5.1/§5.5). Curriculum-side highlights: word-problem/model-method ramp + variation theory (SG), Band-1 check register + whole-child budget + phenomenon weeks (FI), contribution ladder + family meeting + held-struggle Big Problems + cyclical quests (JP). FIX DECISION PENDING with user |
